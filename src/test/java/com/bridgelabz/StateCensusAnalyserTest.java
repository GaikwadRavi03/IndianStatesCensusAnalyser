...
public class StateCensusAnalyserTest {
    ...
    private static final String CSVStateCensusPojo = "com.bridgelabz.CSVStateCensus";
    private static final String CSVStateCodePojo = "com.bridgelabz.CSVStateCode";
    ...
    public void givenTheStatesCSV_fileCheckToEnsure_TheNumberOfRecord_matches() {
        ...
        try {
            String result = StateCensusAnalyser.findStateCount(29, SAMPLE_CSV_STATE_CENSUS_DATA_PATH, CSVStateCensusPojo);
    ...
    public void givenTheState_CSVFileWhenCorrectBut_TypeIncorrect_ReturnsCustomException() {
        String result = null;
        try {
            result = StateCensusAnalyser.findStateCount(29, SAMPLE_CSV_STATE_CODE_PATH, CSVStateCensusPojo);
            Assert.assertEquals("HAPPY", result);
        } catch (StateCensusAnalyserException e) {
    ...
    public void givenTheStateCSV_FileWhenCorrectBut_csv_HeaderIncorrect_ReturnsCustomException() {
        String result = null;
        try {
            result = StateCensusAnalyser.findStateCount(29, SAMPLE_CSV_STATE_CENSUS_DATA_PATH, CSVStateCensusPojo);
        } catch (StateCensusAnalyserException e) {
    ...
    public void givenTheStatesCode_CSV_fileCheckToEnsure_TheNumberOfRecord_matches() {
        String result = null;
        try {
            result = StateCensusAnalyser.findStateCodeCount(37, SAMPLE_CSV_STATE_CODE_PATH, CSVStateCodePojo);
            Assert.assertEquals("HAPPY", result);
        } catch (StateCensusAnalyserException e) {
            e.printStackTrace();
        }
    ...
    public void givenTheStateCode_CSVFileWhenCorrectBut_DelimiterIncorrect_ReturnsCustomException() {
        try {
            String result = StateCensusAnalyser.findStateCodeCount(37, SAMPLE_CSV_STATE_CODE_PATH, CSVStateCodePojo);
        } catch (StateCensusAnalyserException e) {
            Assert.assertEquals(StateCensusAnalyserException.ExceptionType.NO_SUCH_FILE, e.type);
        }
    }
    ...
    //------------------------------
    @Test
    public void givenTheStatesCSV_fileCheckToEnsure_TheNumberOfRecord_matches_R2() throws IOException {

        try {
            String result = StateCensusAnalyser.getStateCount(28, SAMPLE_CSV_STATE_CENSUS_DATA_PATH, CSVStateCensusPojo, SAMPLE_CSV_STATE_CODE_PATH, CSVStateCodePojo);
            Assert.assertEquals("HAPPY", result);
        } catch (StateCensusAnalyserException e) {
        }
    }
